...
export default function ProfileScreen({ navigation }) {
	...
	const [notificationPref, setNotificationPref] = React.useState(initalNotificationPrefState);
	const { logOut, updateUser, globalState, setOnboardingCompleted } = React.useContext(AppContext);
		...
		<TouchableWithoutFeedback onPress={() => Keyboard.dismiss()}>
			<ScrollView>
				<View style={{ padding: 20, flex: 1, gap: 10 }}>
					<Text style={{ fontWeight: '600', fontSize: 20, color: colors.BLACK }}>Personal Information</Text>
					{/* Avatar Section  */}
					<Avatar user={user} />
					{/* Personal Information Section  */}
					<CustomTextInput label="First name" value={firstName} onChangeText={setFirstName} />
					<CustomTextInput value={lastName} label="Last name" placeholder="Doe..." onChangeText={setLastName} />
					<CustomTextInput
						label="Email"
						value={email}
					...
					<Text style={{ fontWeight: '600', fontSize: 15, color: colors.BLACK }}>Email notifications</Text>
					{/* Checkbox Section */}
